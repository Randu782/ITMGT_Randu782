...
import json
with open (r"C:\Users\Marc Philippe\Downloads\script.json") as file:
...
def Collatz(start_number):
    ...
    while start_number != 1:
        ...
        space.append(int(start_number))
        for i in space:
            sum += i
    
          
    return space, max(space), sum
...
def Collatz_winner(number_list):
    ...
    for number in number_list:
        currentchain = Collatz(start_number)
        if currentchain > maxchain and :
            maxnum, maxchain = number, currentchain
