...
def bin_to_dec(binary_string):
    ...
    for digit in range(len_bin):
        if binary_string[len_bin - 1 - digit] == "1":
            decimal += 2 ** digit
...
def dec_to_bin(number):
    ...
    while number > 0:
        modulo = number % 2
        binary_string = str(modulo) + binary_string
        number = number // 2
...
def telephone_cipher(message):
    ...
    for letter in message:
        int_code = encoder_dict[letter]

        if last_int and last_int == int_code[0]:
            space.append("_")

        space.append(int_code)
        last_int = int_code[0]
...
def telephone_decipher(telephone_string):
    ...
    for char in telephone_string:
        if char == '_':
            if current_sequence:
                space.append(decipher_dict[current_sequence])
                current_sequence = ''
        else:
            if current_sequence and char != current_sequence[0]:
                space.append(decipher_dict[current_sequence])
